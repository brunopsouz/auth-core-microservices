...
is_development_environment_variable() {
    case "$1" in
        AUTHCORE_API_PORT|NOTIFICATIONCORE_API_PORT|GATEWAY_API_PORT|ASPNETCORE_ENVIRONMENT|AUTHCORE_POSTGRES_PORT|AUTHCORE_POSTGRES_DB|AUTHCORE_POSTGRES_USER|AUTHCORE_POSTGRES_PASSWORD|AUTHCORE_DATABASE_MIGRATIONS_AUTOMIGRATEONSTARTUP|AUTHCORE_DATABASE_MIGRATIONS_ENSUREDATABASECREATED|NOTIFICATIONCORE_POSTGRES_PORT|NOTIFICATIONCORE_POSTGRES_DB|NOTIFICATIONCORE_POSTGRES_USER|NOTIFICATIONCORE_POSTGRES_PASSWORD|NOTIFICATIONCORE_DATABASE_MIGRATIONS_AUTOMIGRATEONSTARTUP|NOTIFICATIONCORE_DATABASE_MIGRATIONS_ENSUREDATABASECREATED|REDIS_PORT|REDIS_PASSWORD|REDIS__CONNECTIONSTRING|REDIS__KEYPREFIX|RABBITMQ_PORT|RABBITMQ_MANAGEMENT_PORT|RABBITMQ__HOST|RABBITMQ__PORT|RABBITMQ__VIRTUALHOST|RABBITMQ__USERNAME|RABBITMQ__PASSWORD|RABBITMQ__EXCHANGE|RABBITMQ__ROUTINGKEY|RABBITMQ__QUEUE|RABBITMQ__DEADLETTERQUEUE|SMTP_HOST|SMTP_CONTAINER_PORT|SMTP_PORT|SMTP_WEB_PORT|SMTP_USERNAME|SMTP_PASSWORD|SMTP_USE_TLS|SMTP_SENDER_EMAIL|SMTP_SENDER_NAME|SMTP_TIMEOUT_SECONDS|CONNECTIONSTRINGS__POSTGRESQL|AUTHENTICATION__JWT__ISSUER|AUTHENTICATION__JWT__AUDIENCE|AUTHENTICATION__JWT__SIGNINGKEY|AUTHENTICATION__JWT__ACCESSTOKENLIFETIMEMINUTES|AUTHENTICATION__JWT__REFRESHTOKENLIFETIMEDAYS|AUTHENTICATION__JWT__CLOCKSKEWSECONDS|AUTHENTICATION__GOOGLE__CLIENTID|AUTHENTICATION__GOOGLE__CLIENTSECRET|AUTHENTICATION__GOOGLE__CALLBACKPATH|AUTHENTICATION__DEFAULTRETURNURL|AUTHENTICATION__ALLOWEDRETURNURLS__0|AUTHENTICATION__ALLOWEDRETURNURLS__1|AUTHENTICATION__ALLOWEDRETURNURLS__2|AUTHENTICATION__ALLOWEDRETURNURLS__3|AUTH__COOKIE__SESSIONCOOKIENAME|AUTH__COOKIE__ACCESSTOKENCOOKIENAME|AUTH__CSRF__SIGNINGKEY|AUTH__CSRF__ALLOWEDORIGINS__0|AUTH__CSRF__ALLOWEDORIGINS__1|AUTH__CSRF__ALLOWEDORIGINS__2|AUTH__CSRF__ALLOWEDORIGINS__3|OUTBOX__ENABLED|OUTBOX__BATCHSIZE|OUTBOX__POLLINGINTERVALSECONDS|OUTBOX__MAXATTEMPTS|NOTIFICATIONCORE_RABBITMQ_CONSUMER_ENABLED|NOTIFICATIONCORE_DISPATCHER_ENABLED|NOTIFICATIONCORE_DISPATCHER_BATCH_SIZE|NOTIFICATIONCORE_DISPATCHER_POLLING_INTERVAL_SECONDS|NOTIFICATIONCORE_DISPATCHER_RETRY_DELAY_SECONDS|NOTIFICATIONCORE_DISPATCHER_PROCESSING_TIMEOUT_SECONDS)
            return 0
            ;;
        *)
            return 1
...
validate_local_composed_configuration() {
    local expected_postgresql_connection
    local expected_redis_connection

    expected_postgresql_connection="Host=localhost;Port=${AUTHCORE_POSTGRES_PORT};Database=${AUTHCORE_POSTGRES_DB};Username=${AUTHCORE_POSTGRES_USER};Password=${AUTHCORE_POSTGRES_PASSWORD};Pooling=true;Minimum Pool Size=0;Maximum Pool Size=30;Timeout=10;Command Timeout=30;Application Name=AuthCore"
    expected_redis_connection="localhost:${REDIS_PORT},password=${REDIS_PASSWORD},abortConnect=false"

    if [ "$CONNECTIONSTRINGS__POSTGRESQL" != "$expected_postgresql_connection" ]; then
        echo -e "${RED}Erro:${NC} 'CONNECTIONSTRINGS__POSTGRESQL' nao corresponde as credenciais locais do PostgreSQL em $ENV_FILE."
        exit 1
    fi

    if [ "$REDIS__CONNECTIONSTRING" != "$expected_redis_connection" ]; then
        echo -e "${RED}Erro:${NC} 'REDIS__CONNECTIONSTRING' nao corresponde as credenciais locais do Redis em $ENV_FILE."
        exit 1
    fi
}
...
require_infrastructure_environment() {
    ...
    require_environment_value "RABBITMQ__EXCHANGE"
    require_environment_value "RABBITMQ__ROUTINGKEY"
    require_environment_value "RABBITMQ__QUEUE"
    require_environment_value "RABBITMQ__DEADLETTERQUEUE"
...
require_api_environment() {
    ...
    require_environment_value "CONNECTIONSTRINGS__POSTGRESQL"
    require_environment_value "REDIS__CONNECTIONSTRING"
    ...
    require_environment_value "AUTHENTICATION__JWT__REFRESHTOKENLIFETIMEDAYS"
    require_environment_value "AUTHENTICATION__JWT__CLOCKSKEWSECONDS"
    require_environment_value "AUTH__COOKIE__SESSIONCOOKIENAME"
    require_environment_value "AUTH__COOKIE__ACCESSTOKENCOOKIENAME"
    require_environment_value "AUTH__CSRF__SIGNINGKEY"
    require_environment_value "RABBITMQ__HOST"
    require_environment_value "RABBITMQ__PORT"
    require_environment_value "RABBITMQ__VIRTUALHOST"
    require_environment_value "OUTBOX__ENABLED"
    require_environment_value "OUTBOX__BATCHSIZE"
    require_environment_value "OUTBOX__POLLINGINTERVALSECONDS"
    require_environment_value "OUTBOX__MAXATTEMPTS"
