...
public sealed class NotificationRepositoryPersistenceTests : IClassFixture<PostgreSqlIntegrationFixture>
{
    ...
    public async Task Persistence_WhenNotificationLifecycleChanges_ShouldPersistAndLoadState()
    {
        ...
        await using var scope = _fixture.Services.CreateAsyncScope();
        var unitOfWork = scope.ServiceProvider.GetRequiredService<IUnitOfWork>();
        var notificationWriterRepository = scope.ServiceProvider.GetRequiredService<INotificationWriterRepository>();
        var notificationReadRepository = scope.ServiceProvider.GetRequiredService<INotificationReadRepository>();
        var notificationSearchRepository = scope.ServiceProvider.GetRequiredService<INotificationSearchRepository>();
        var notificationDispatchRepository = scope.ServiceProvider.GetRequiredService<INotificationDispatchRepository>();
        ...
        await unitOfWork.BeginTransactionAsync();
        Assert.True(await notificationWriterRepository.TryAddAsync(notification));
        await unitOfWork.CommitAsync();

        Assert.False(await notificationWriterRepository.TryAddAsync(notification));

        var persisted = await notificationReadRepository.GetByIdAsync(notification.Id);
        ...
        var pending = await notificationDispatchRepository.GetPendingForDispatchAsync(requestedAtUtc, take: 10);
        ...
        await notificationWriterRepository.UpdateAsync(persisted);

        var timedOut = await notificationDispatchRepository.GetProcessingTimedOutAsync(requestedAtUtc.AddMinutes(16), take: 10);

        Assert.Contains(timedOut, found => found.Id == notification.Id);

        var processing = await notificationReadRepository.GetByIdempotencyKeyAsync(idempotencyKey);
        ...
        processing!.RegisterSuccessfulDeliveryAttempt(
            ...
            "provider-message-1");
        await notificationWriterRepository.UpdateAsync(processing);

        var sent = await notificationReadRepository.GetByIdAsync(notification.Id);
        ...
        var searchResult = await notificationSearchRepository.SearchAsync(
    ...
    public async Task Rollback_WhenNotificationIsAddedInTransaction_ShouldDiscardChanges()
    {
        ...
        await unitOfWork.BeginTransactionAsync();
        await notificationWriterRepository.TryAddAsync(notification);
        await unitOfWork.RollbackAsync();

        var persisted = await notificationReadRepository.GetByIdempotencyKeyAsync(idempotencyKey);
